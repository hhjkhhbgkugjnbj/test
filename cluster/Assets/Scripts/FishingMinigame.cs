...
public class FishingMinigame : MonoBehaviour
{
    ...
    public Text popupText; // Text component within the popup panel
	
	private void Start() {
		FishManager.InitFishManager();
	...
	public void FishCaught() {
		...
		// Display popup with fish name
		ShowPopup($"{currentFishOnLine.name}을(를) 낚았다!");

		//학생증 낚으면 미니게임 종료 과정 -> 미니게임 종료 신호
		if (currentFishOnLine.name == "학생증")
        {
	...
	private IEnumerator EndGameAfterDelay()
	{
		...
		position_update_info.pt_id = PROTOCOL.MiniGame_End_Request;
	...
	private void ShowPopup(string message) {
        ...
        Invoke("HidePopup", 2f); // Uncomment this if you want to hide it after 2 seconds
    }
